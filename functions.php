...
function kraken_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'kraken' ),
		)
	);
...
function kraken_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'kraken' ),
...
function kraken_scripts() {
	wp_enqueue_style( 'kraken-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'kraken-style', 'rtl', 'replace' );

	wp_enqueue_script( 'kraken-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
